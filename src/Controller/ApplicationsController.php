...
class ApplicationsController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $application = $this->Applications->get($id, contain: ['Jobs', 'Applicants']);
    ...
    public function add()
    {
        ...
        $jobs = $this->Applications->Jobs->find('list', ['limit' => 200])->all();
        $applicants = $this->Applications->Applicants->find('list', ['limit' => 200])->all();
        $this->set(compact('application', 'jobs', 'applicants'));
    }
    ...
    public function updateStatus($id = null)
{
    ...
    if ($this->request->is('post')) {
        // Patch the application with the form data (including the updated status)
        $application = $this->Applications->patchEntity($application, $this->request->getData());
        ...
        if ($this->Applications->save($application)) {
            ...
            $this->Flash->error('Unable to update status. Please try again.');
        }

        // Redirect to the index page to show the updated status in the list
        return $this->redirect(['action' => 'index']);
    }

    // If it's not a POST request, just redirect to the index page (this shouldn't happen in this case)
    return $this->redirect(['action' => 'index']);
}
